...
// Users may disable SIMD with EGNX_NO_SIMD, which may be set via compiler flags.
// The gn build has no option which sets EGNX_NO_SIMD.
// Use EGVX_USE_SIMD internally to avoid confusing double negation.
// Do not use 'defined' in a macro expansion.
#if !defined(EGNX_NO_SIMD)
    #define EGVX_USE_SIMD 1
#else
    #define EGVX_USE_SIMD 0
#endif
...
#if defined(_MSC_VER)
    #define EGVX_ALWAYS_INLINE __forceinline
#else
    #define EGVX_ALWAYS_INLINE __attribute__((always_inline))
#endif
...
struct alignas(N*sizeof(T)) VecStorage {
    EGVX_ALWAYS_INLINE VecStorage() = default;
    EGVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
...
struct VecStorage<4,T> {
    EGVX_ALWAYS_INLINE VecStorage() = default;
    EGVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
    EGVX_ALWAYS_INLINE VecStorage(T x, T y, T z, T w) : lo(x,y), hi(z, w) {}
    EGVX_ALWAYS_INLINE VecStorage(Vec<2,T> xy, T z, T w) : lo(xy), hi(z,w) {}
    EGVX_ALWAYS_INLINE VecStorage(T x, T y, Vec<2,T> zw) : lo(x,y), hi(zw) {}
    EGVX_ALWAYS_INLINE VecStorage(Vec<2,T> xy, Vec<2,T> zw) : lo(xy), hi(zw) {}

    EGVX_ALWAYS_INLINE Vec<2,T>& xy() { return lo; }
    EGVX_ALWAYS_INLINE Vec<2,T>& zw() { return hi; }
    EGVX_ALWAYS_INLINE T& x() { return lo.lo.val; }
    EGVX_ALWAYS_INLINE T& y() { return lo.hi.val; }
    EGVX_ALWAYS_INLINE T& z() { return hi.lo.val; }
    EGVX_ALWAYS_INLINE T& w() { return hi.hi.val; }

    EGVX_ALWAYS_INLINE Vec<2,T> xy() const { return lo; }
    EGVX_ALWAYS_INLINE Vec<2,T> zw() const { return hi; }
    EGVX_ALWAYS_INLINE T x() const { return lo.lo.val; }
    EGVX_ALWAYS_INLINE T y() const { return lo.hi.val; }
    EGVX_ALWAYS_INLINE T z() const { return hi.lo.val; }
    EGVX_ALWAYS_INLINE T w() const { return hi.hi.val; }

    // Exchange-based swizzles. These should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
    EGVX_ALWAYS_INLINE Vec<4,T> yxwz() const { return shuffle<1,0,3,2>(bit_pun<Vec<4,T>>(*this)); }
    EGVX_ALWAYS_INLINE Vec<4,T> zwxy() const { return shuffle<2,3,0,1>(bit_pun<Vec<4,T>>(*this)); }
...
struct VecStorage<2,T> {
    EGVX_ALWAYS_INLINE VecStorage() = default;
    EGVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
    EGVX_ALWAYS_INLINE VecStorage(T x, T y) : lo(x), hi(y) {}

    EGVX_ALWAYS_INLINE T& x() { return lo.val; }
    EGVX_ALWAYS_INLINE T& y() { return hi.val; }

    EGVX_ALWAYS_INLINE T x() const { return lo.val; }
    EGVX_ALWAYS_INLINE T y() const { return hi.val; }

    // This exchange-based swizzle should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
    EGVX_ALWAYS_INLINE Vec<2,T> yx() const { return shuffle<1,0>(bit_pun<Vec<2,T>>(*this)); }

    EGVX_ALWAYS_INLINE Vec<4,T> xyxy() const {
...
struct alignas(N*sizeof(T)) Vec : public VecStorage<N,T> {
    ...
    EGVX_ALWAYS_INLINE Vec() = default;
    ...
    EGVX_ALWAYS_INLINE Vec(std::initializer_list<T> xs) {
        T vals[N] = {0};
    ...
    EGVX_ALWAYS_INLINE T  operator[](int i) const { return i<N/2 ? this->lo[i] : this->hi[i-N/2]; }
    EGVX_ALWAYS_INLINE T& operator[](int i)       { return i<N/2 ? this->lo[i] : this->hi[i-N/2]; }

    EGVX_ALWAYS_INLINE static Vec Load(const void* ptr) {
        Vec v;
        memcpy(&v, ptr, sizeof(Vec));
        return v;
    }
    EGVX_ALWAYS_INLINE void store(void* ptr) const {
...
struct Vec<1,T> {
    ...
    EGVX_ALWAYS_INLINE Vec(std::initializer_list<T> xs) : val(xs.size() ? *xs.begin() : 0) {}

    EGVX_ALWAYS_INLINE T  operator[](int) const { return val; }
    EGVX_ALWAYS_INLINE T& operator[](int)       { return val; }
...
// or often integrate them directly into the recursion of style 3), allowing fine control.
...
#if EGVX_USE_SIMD && (defined(__clang__) || defined(__GNUC__))
    ...
    // Either EGNX_NO_SIMD is defined, or Clang/GCC vector extensions are not available.
...
#if EGVX_USE_SIMD && defined(__AVX2__)
...
#endif
#if EGVX_USE_SIMD && defined(__SSE4_1__)
...
#endif
#if EGVX_USE_SIMD && defined(__ARM_NEON)
    if constexpr (N*sizeof(T) == 16) {
...
#endif
#if EGVX_USE_SIMD && defined(__SSE_4_1__)
    if constexpr (N*sizeof(T) == 16) {
        return !_mm_testz_si128(bit_pun<__m128i>(x), _mm_set1_epi32(-1));
    }
#endif
#if EGVX_USE_SIMD && defined(__SSE__)
    if constexpr (N*sizeof(T) == 16) {
        // On SSE, movemask checks only the MSB in each lane, which is fine if the lanes were set
        // directly from a comparison op (which sets all bits to 1 when true), but egvx::Vec<>
...
#endif
#if EGVX_USE_SIMD && defined(__aarch64__)
...
#endif
#if EGVX_USE_SIMD && defined(__wasm_simd128__)
...
#if EGVX_USE_SIMD && defined(__clang__)
...
// pin matches the logic of EgTPin, which is important when NaN is involved. It always returns
...
SI auto map(std::index_sequence<I...>,
            Fn&& fn, const Args&... args) -> egvx::Vec<sizeof...(I), decltype(fn(args[0]...))> {
    auto lane = [&](size_t i)
#if defined(__clang__)
...
#if EGVX_USE_SIMD && defined(__AVX__)
    if constexpr (N == 8) {
...
#if EGVX_USE_SIMD && defined(__F16C__)
    if constexpr (N == 8) {
...
SINT std::enable_if_t<std::is_unsigned_v<T>, Vec<N,T>> saturated_add(const Vec<N,T>& x,
                                                                     const Vec<N,T>& y) {
#if EGVX_USE_SIMD && (defined(__SSE__) || defined(__ARM_NEON))
...
#else
    return egvx::cast<uint16_t>(mull(x, y) >> 16);
#endif
}
...
#elif EGVX_USE_SIMD && defined(__SSE__)
